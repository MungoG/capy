...
================================================================================
    ...
    1. UTF-8 Everywhere
       ...
       - Public interface uses `char` and `std::string`
       - Accepts wchar_t/wstring and char8_t/u8string inputs (converted on construction)
    ...
    5. Allocator Support
       - Constructors accepting polymorphic allocators are provided
       - Enables use with custom memory resources

================================================================================
OPEN DESIGN QUESTIONS
================================================================================

    1. Is path_view Necessary?
       -----------------------
       path_view provides a non-owning reference with the same validity
       guarantee as path. However, since paths are typically short and
       validation is required anyway, the utility over std::string_view
       may be limited. Consider whether the added complexity is justified.

       Arguments for path_view:
       - Type safety: prevents accidentally passing arbitrary strings
       - Decomposition returns views without allocation
       - Consistent with string/string_view pattern

       Arguments against:
       - Additional type to maintain
       - Users must learn when to use path vs path_view
       - Validation cost on construction

    2. Internal Storage: std::string vs std::u8string
       ----------------------------------------------
       Currently uses std::string. Alternatives considered:

       std::string (current):
       + Works with existing code expecting char*
       + No friction with I/O, logging, etc.
       + char8_t ecosystem still immature
       - Type doesn't encode the UTF-8 invariant

       std::u8string:
       + Type explicitly documents UTF-8 encoding
       + Prevents accidental mixing with other encodings
       - Requires reinterpret_cast for most APIs
       - char8_t poorly supported by standard library
       - Would leak into public interface, causing friction

       Recommendation: Stay with std::string until char8_t ecosystem matures.

    3. Is native_string() Required?
       ----------------------------
       Modern Windows APIs accept forward slashes in most contexts.
       However, some scenarios still require backslashes:
       - Command-line argument parsing
       - Some third-party libraries
       - Display to users (convention)
       - UNC paths (\\server\share)

       Keep native_string() but document that forward slashes work
       for most Windows API calls.
...
================================================================================
    ...
    class path:
        ...
        - May optionally use polymorphic allocator
...
================================================================================
    ...
    6. path(std::wstring_view s)
       - Convert UTF-16/wide to UTF-8
       - Convert backslashes to forward slashes
       - Validate result
       - If invalid, throw system_error

    7. path(wchar_t const* s)
       - Equivalent to path(std::wstring_view(s))

    8. path(std::u8string_view s)
       - Treat as UTF-8, convert char8_t to char
       - Validate
       - If invalid, throw system_error

    9. path(char8_t const* s)
       - Equivalent to path(std::u8string_view(s))

    10. path(std::string_view s, std::pmr::polymorphic_allocator<char> alloc)
        - Allocator-aware construction

    11. path(unchecked_t, std::string&& s) [private]
...
================================================================================
    ...
    Note: Modern Windows APIs generally accept forward slashes. The native
    conversion functions are provided for compatibility with older APIs,
    third-party libraries, user display, and UNC paths.
...
================================================================================
    ...
    filesystem_path():
    ------------------
    Returns a std::filesystem::path for use with APIs that require it.
    On POSIX, constructs directly from the UTF-8 string.
    On Windows, constructs from the wide string to avoid encoding issues.

    Implementation:
        std::filesystem::path filesystem_path() const {
        #ifdef _WIN32
            return std::filesystem::path(native_wstring());
        #else
            return std::filesystem::path(s_);
        #endif
        }
...
================================================================================
    ...
    1. No encoding ambiguity
       ...
       - Accepts wide/u8 strings, converts on construction
       - No u16string(), u32string() - just string()
    ...
    7. Allocator support
       - Polymorphic allocator constructors available

    8. Interoperability
       - filesystem_path() returns std::filesystem::path for legacy APIs
...
public:
    ...
    */
    ...
    // Comparison
    //
    // Note: All comparisons are bitwise (byte-by-byte) on the
    // underlying UTF-8 strings. Paths are not normalized before
    // comparison, so "foo//bar" != "foo/bar" and "FOO" != "foo".
    //
    //--------------------------------------------

    /** Compare this path to another.

        Performs bitwise comparison of the underlying UTF-8 strings.
...
public:
    ...
    */
    ...
    //--------------------------------------------
    //
    // Wide string construction (converts from UTF-16/wide to UTF-8)
    //
    //--------------------------------------------

    /** Construct from wide string_view, converts and validates.

        Converts from UTF-16/wide encoding to UTF-8.
        Backslashes are converted to forward slashes.

        @throws system_error on invalid path
    */
    explicit path(std::wstring_view s);

    /** Construct from null-terminated wide string, converts and validates.
        ...
        @throws system_error on invalid path
    */
    explicit path(wchar_t const* s);

    /** Construct from wide string, converts and validates.
        ...
        @throws system_error on invalid path
    */
    explicit path(std::wstring const& s);

    //--------------------------------------------
    //
    // char8_t construction (C++20)
    //
    //--------------------------------------------

    /** Construct from u8string_view, validates and throws on invalid.

        @throws system_error on invalid path
    */
    explicit path(std::u8string_view s);

    /** Construct from null-terminated char8_t string, validates and throws on invalid.

        @throws system_error on invalid path
    */
    explicit path(char8_t const* s);

    /** Construct from u8string, validates and throws on invalid.

        @throws system_error on invalid path
    */
    explicit path(std::u8string const& s);

    /** Construct from u8string (moves if valid), validates and throws on invalid.

        @throws system_error on invalid path
    */
    explicit path(std::u8string&& s);
    ...
    /** Construct with allocator (empty path).
    */
    explicit path(allocator_type alloc);

    /** Construct from string_view with allocator.

        @throws system_error on invalid path
    */
    path(std::string_view s, allocator_type alloc);

    /** Construct from path_view with allocator.
    */
    path(path_view pv, allocator_type alloc);

    /** Construct from wide string_view with allocator.

        @throws system_error on invalid path
    */
    path(std::wstring_view s, allocator_type alloc);

    /** Construct from u8string_view with allocator.

        @throws system_error on invalid path
    */
    path(std::u8string_view s, allocator_type alloc);
    ...
    */
    ...
    /** Return the path in native format.
        ...
        @note Modern Windows APIs generally accept forward slashes.
              This function is provided for compatibility with older
              APIs, third-party libraries, and user display.
    */
    ...
    */
    ...
    //--------------------------------------------
    //
    // std::filesystem interoperability
    //
    //--------------------------------------------

    /** Return a std::filesystem::path for use with standard library APIs.

        On POSIX, constructs directly from the UTF-8 string.
        On Windows, constructs from the wide string to avoid encoding issues.
    */
    std::filesystem::path
    filesystem_path() const
    {
#ifdef _WIN32
        return std::filesystem::path(native_wstring());
#else
...
/** Test if two paths are equivalent using platform rules.
    ...
    (e.g., U+00E9 "é" vs U+0065 U+0301 "e + combining accent") will
